...
public class YamlConfiguration {
    ...
    private static final String DEFAULT_CONFIGURATION = "qfrag.default.yaml";
    private static final String DEFAULT_CUSTOM_CONFIGURATION = "qfrag.yaml";
...
/*                    .put("worker_context", new GiraphClassConfigurationAssignment<WorkerContext>() {
                    ...
                    .put("communication_strategy", new GiraphConfigurationAssignment() {
                        ...
                        public void assign(String propertyKey, Object propertyValue, GiraphConfiguration giraphConfiguration) {
                            ...
                            giraphConfiguration.set(qfrag.conf.Configuration.CONF_COMM_STRATEGY_FACTORY_CLASS, strategyFactoryClass);
                        }
                    })
                    ...
                    // Output
                    .put("output_active", new GiraphBooleanConfigurationAssignment(qfrag.conf.Configuration.CONF_OUTPUT_ACTIVE))
                    .put("output_path", new GiraphStringConfigurationAssignment(qfrag.conf.Configuration.CONF_OUTPUT_PATH))
    ...
    public YamlConfiguration(String[] args) {
        ...
        List<String> list = findUserProvidedConfigs(args);
        System.out.println("User provided configs are:\n");
        for(String arg: list)
    ...
    public void load() {
        for (String configPath : configurations) {
            System.out.println("Will load from " + configPath);
    ...
    private void addPrivateConfiguration(GiraphConfiguration configuration) {
        ...
        //configuration.setComputationClass(qfrag.computation.QFragEngine.class);
...
/*        if (GiraphConstants.WORKER_CONTEXT_CLASS.get(configuration).equals(DefaultWorkerContext.class)) {
    ...
    private List<String> findUserProvidedConfigs(String[] args) {
        ...
        try {
            ...
            if (cmd.hasOption("y")) {
                System.out.println("Command line options contain y option = " + cmd.getOptionValues("y"));
                return Arrays.asList(cmd.getOptionValues("y"));
            } else {
                System.out.println("Command line options do not contain y option = " + cmd.getOptionValues("y"));
                System.out.println("Setting the default option: " + DEFAULT_CUSTOM_CONFIGURATION);
    ...
    private URL getConfigUrl(String urlStr) {
        URL url;
        System.out.println("Passed url = " + urlStr);
        try {
            url = new URL(urlStr);
